...
// 1. Write a function called first that returns the first item of the array using a callback function
  ...
  // Code Here
  function first(namely, checkFirst) {
    checkFirst(namely[0]);
  }
...
// 2. Write a function called last which returns the last item of the array using a callback function.
  ...
  //Code Here
  function last(namely, lastly) {
    lastly(namely[namely.length-1]);
  }
...
// If it does, return true using the callback, if not return false.
  ...
  //Code Here
  function contains(namely, namecontain, func) {
      func(namely.indexOf(namecontain) !== -1);
  }
...
// the callback function with the array of unique names.
    ...
    //Code Here
    function uniq(namely, func){
      var namelyUniq = [];
      for (var i=0;i<namely.length;i++) {
        if (namelyUniq.indexOf(namely[i]) === -1) {
          namelyUniq.push(namely[i]);
        }
      }
      func(namelyUniq);
    }
...
// and returns that user.
 ...
 //Code Here
 function getUserById (userly,ide, func) {
   for (var i=0;i<userly.length;i++) {
     if (userly[i].id===ide) {
       func(userly[i]);
     }
   }
